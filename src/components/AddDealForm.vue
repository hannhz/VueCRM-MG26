...
<template>
  ...
  <Transition name="slide-deal">
    ...
    >
      ...
      <div
        class="w-200 h-full flex-shrink-0"
        style="background: linear-gradient(to right, transparent, rgba(100,114,139,0.6))"
      ></div>
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="formData.dealName" type="text" placeholder="Deal's Name"
              ...
              <div>
                ...
                <select v-model="formData.pipeline"
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input v-model="formData.amount" type="number" placeholder="Amount/Value"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="formData.expectedCloseDate" type="date" placeholder="Close Date"
                  class="w-full px-3 py-2 border border-outline rounded-lg focus:outline-none focus:ring-1 focus:ring-sub-text text-sm" />
              </div>
              <div>
            ...
            <div class="border border-outline rounded-lg overflow-hidden">
              ...
              >
                <div
                  class="w-4 h-4 border-2 rounded flex items-center justify-center flex-shrink-0 transition-colors"
                  :class="formData.showOptional ? 'border-sub-text bg-sub-text' : 'border-outline'"
                >
                  <svg v-if="formData.showOptional" class="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 10 8">
                    <path d="M1 4l3 3 5-6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </div>
                <span class="text-sub-text font-medium">Description & Document (Opsional)</span>
              </button>

              <div v-if="formData.showOptional" class="px-4 pb-4 space-y-4 border-t border-outline pt-4">
                <!-- Description -->
                <div>
                ...
                <div>
                  ...
                  <select v-model="formData.documents"
                    class="w-full px-3 py-2 border border-outline rounded-lg focus:outline-none focus:ring-1 focus:ring-sub-text text-sm bg-white">
                    <option v-for="opt in fileSourceOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                  </select>
                  ...
                  <div v-if="formData.documents === 'local'" class="mt-3">
                    ...
                    <ul v-if="selectedFiles.length" class="mt-2 space-y-1">
                      ...
                      >
                        ...
                        <button type="button" @click="removeFile(i)" class="ml-2 text-sub-text hover:text-red-500 flex-shrink-0">✕</button>
                      </li>
                    </ul>
                  </div>
                </div>
