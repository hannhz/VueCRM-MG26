<template>
  <div class="min-h-screen bg-gray-100 flex items-center justify-center p-6">
    ...
    >
      ...
      >
        <h1 class="text-5xl font-extrabold text-blue-950 mb-6 tracking-tight">CRM MG26</h1>
        ...
        <!-- Illustration -->
        <img src="/public/login.svg" alt="illustration" class="w-80 mt-10 drop-shadow-md" />
      </div>
      ...
      <div class="p-10 lg:p-16 flex flex-col justify-center bg-white">
        ...
        <form @submit.prevent="submit">
          ...
          <div class="relative mb-6 group">
            <input
              ...
              :disabled="isLoading"
            />
            <span class="absolute right-4 top-1/2 -translate-y-1/2 opacity-50 group-focus-within:opacity-100 transition-opacity">
          ...
          >
            <template v-if="isLoading">
              <span class="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full font-bold"></span>
              <span>Memproses...</span>
            </template>
            <template v-else>
              <span>Log In</span>
        ...
        <p class="text-center text-slate-500 mt-10 font-medium">
          ...
          <span class="text-blue-950 font-bold cursor-pointer hover:underline underline-offset-4 decoration-2">
            Sign Up
          </span>
        </p>
      </div>
...
const submit = async () => {
  ...
  try {
    // 1. Auth sign in (Using flat data to match credentials object)
    await store.dispatch("auth/authsignin", {
      email: email.value,
      password: password.value
    });
    
    // 2. Clear tabs and menu settings (from senior's pattern)
    store.dispatch("tabs/clearTabs");
    store.dispatch("settingsfe/clearmenuuser");
    
    // 3. Fetch layout/menu configuration
    const data = await store.dispatch("settingsfe/actlayoutweb");
    
    // 4. Handle navigation based on defaults
    if (data && data.dbmenu2 && data.dbmenu2.length > 0) {
      const defaultPath = data.dbmenu2[0].pathfiledefault || data.dbmenu2[0].pathfile;
      
      // Filter for default menu item to open in tabs
      let filter = data.dbmenu2.filter(
        (t) => t.pathfile === defaultPath
      );
      
      if (filter.length > 0) {
        store.dispatch("tabs/handleOpenTab", filter[0]);
      }
